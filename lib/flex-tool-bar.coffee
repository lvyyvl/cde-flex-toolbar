...
module.exports =
  ...
  activate: ->
    require('atom-package-deps').install('cde-flex-toolbar')
  ...
  resolveConfigPath: ->
    @configFilePath = atom.config.get 'cde-flex-toolbar.toolBarConfigurationFilePath'
    ...
    unless fs.isFileSync(@configFilePath)
      @configFilePath = fs.resolve @configFilePath, 'cdetoolbar', ['cson', 'json5', 'json', 'js', 'coffee']

    return true if @configFilePath

    unless @configFilePath
      @configFilePath = path.join atom.configDirPath, 'cdetoolbar.cson'
      defaultConfig = '''
...
[
  {
    ...
    tooltip: "Create React Native Project"
  }
  {
    type: "button"
    iconset: "mdi"
  ...
  }
  {
    ...
    tooltip: "Add Folder in Current Window"
  }
  {
    type: "button"
    iconset: "fa"
...
'''
    ...
    if editor?.buffer?.file?.getParent()?.path?
      ...
      while count < projectCount
        ...
        if editor.buffer.file.getParent().path.includes(pathToCheck)
          @projectToolbarConfigPath = fs.resolve pathToCheck, 'cdetoolbar', ['cson', 'json5', 'json', 'js', 'coffee']
        count++
  ...
  registerCommand: ->
    @subscriptions.add atom.commands.add 'atom-workspace',
      'CDE-Flex-Toolbar:Edit-Config-File': =>
  ...
  registerWatch: ->
    if atom.config.get('cde-flex-toolbar.reloadToolBarWhenEditConfigFile')
  ...
  consumeToolBar: (toolBar) ->
    @toolBar = toolBar 'cde-flex-toolBar'
    @reloadToolbar(false)
  ...
  reloadToolbar: (withNotification=false) ->
    ...
    try
      ...
      @unfixToolBarHeight()
      atom.notifications.addError 'Your `cdetoolbar.cson` is **not valid CSON**!'
      console.error error
